...
for y, line in enumerate(lines):
    for x, cube in enumerate(line):
        isActive = cube == "#"
        mapCubes[(x, y, z, w)] = isActive
        
neighbours = [x for x in itertools.product([-1,0,1], repeat=4)]
...
def getNeighbours(cube):
    ...
    for dx, dy, dz, dw in neighbours:
        adjCubes.append((x+dx, y+dy, z+dz, w+dw))
    return adjCubes
...
print("Part 2 - total active cubes:",list(mapCubes.values()).count(True))